...
def call_llm(system_prompt: str, user_prompt: str) -> str:
    ...
    if not _llm_announced:
        print("[LLM] 모듈 버전: 2026-09-14-v10 (HF 모델: openai/gpt-oss-120b:fastest, 캡션/이미지프롬프트 개편)")
        _llm_announced = True
    last_error = None
...
def generate_card_content(article_title: str, article_summary: str, country: str, category: str = "") -> dict:
    tone = TONE_GUIDE.get(country, "")
    need_translation = country != "KR"

    # 추상적/수치 중심 주제(경제 등)는 인포그래픽 스타일, 그 외는 사실적 매거진 화보 스타일로 분기
    infographic_categories = {"경제", "정책/혜택/제도", "날씨"}
    if category in infographic_categories:
        style_guide = (
            "이 주제는 수치/통계/추상적 개념 중심이다. 이미지 프롬프트는 "
            "'clean modern infographic style, minimal flat design, data visualization elements "
            "like charts/graphs/icons, editorial magazine infographic' 같은 인포그래픽 스타일로 작성하라."
        )
    else:
        style_guide = (
            "이미지 프롬프트는 실제 잡지 화보처럼 사실적인 사진 스타일로 작성하라. "
            "'photorealistic editorial magazine photography, professional lighting, shallow depth of field, "
            "high-end lifestyle magazine cover style' 같은 표현을 포함하라."
        )
...
{{
  "headline": "2~3줄로 자연스럽게 줄바꿈 가능한 헤드라인. 이모지 절대 사용 금지.",
  "image_prompt": "{style_guide} 인물 얼굴을 특정 인물로 특정하지 않을 것. 이미지 안에 어떤 글자/텍스트/워터마크도 들어가지 않도록 'no text, no letters, no watermark, no words'를 프롬프트에 반드시 포함할 것. 영어로 작성.",
  "caption": "게시글 본문. 인스타그램 모바일 화면 기준 11~15줄 정도 분량으로 충분히 길게 작성(한국어는 약 400~600자, 영어/일본어는 그에 준하는 분량). 반말/캐주얼한 구어체 금지, 정돈된 매거진 톤의 격식 있는 문장으로 작성. 이모지 사용 금지.",
  "alt_text": "인스타그램 대체텍스트 규정에 맞게 작성: (1)이미지 속 사물/개념/장소를 구체적으로 설명하고 (2)이미지 안에 텍스트가 있다면 그 텍스트를 그대로 포함하며 (3)2~3문장 이내로 작성.",
  "hashtag": "#해시태그1개",
  {translation_field}
  "_end": true
}}
