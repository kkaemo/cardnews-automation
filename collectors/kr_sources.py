...
def fetch_naver_news(limit_per_query: int = 8) -> list:
    ...
    for q in NAVER_SEED_QUERIES:
        ...
        res.encoding = "utf-8"  # 네이버 API가 charset을 명시 안 해 한글이 깨지는 문제 방지
...
def _naver_search(query: str, client_id: str, client_secret: str, display: int = 5) -> list:
    headers = {"X-Naver-Client-Id": client_id, "X-Naver-Client-Secret": client_secret}
    res = requests.get(
        "https://openapi.naver.com/v1/search/news.json",
        headers=headers, params={"query": query, "display": display, "sort": "sim"}, timeout=10,
    )
    res.raise_for_status()
    res.encoding = "utf-8"
    items = res.json().get("items", [])
...
def enrich_kr_with_naver(topic_title: str, top_n: int = 3) -> str:
    """
    웰로에서 뽑힌 화제 제목을 네이버에 검색해서 실제 원문 기사를 찾아온다.
    - 1차: 제목 그대로 검색
    - 검색 결과가 0건이면 뒤에서부터 단어를 하나씩 빼며 재검색
      (예: "홍상수 감독, 본처와의 갈등 재점화 논란" → "홍상수 감독 본처와의")
    - 찾은 기사 중 상위 top_n건의 제목+요약을 합쳐서 본문 재료로 반환
    - NAVER_CLIENT_ID/SECRET이 없으면 조용히 빈 문자열 반환 (필수 아님, 있으면 보강)
    """
    client_id = os.environ.get("NAVER_CLIENT_ID")
    client_secret = os.environ.get("NAVER_CLIENT_SECRET")
    if not client_id or not client_secret:
    ...
    try:
        ...
        for cut in range(len(words), 1, -1):
            ...
            results = _naver_search(query, client_id, client_secret, display=top_n)
            if results:
                break
        if not results:
            return ""
        combined = "\n\n".join(f"[기사{i+1}] {r['title']} - {r['desc']}" for i, r in enumerate(results[:top_n]))
        return combined
    except Exception as e:
        print(f"[KR][네이버 보강검색] 실패({topic_title[:20]}...): {e}")
        return ""
