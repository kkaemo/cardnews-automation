...
from collectors import kr_sources, jp_sources, us_sources, base
...
def process_country(country: str, raw_articles: list) -> tuple:
    ...
    for a in top5:
        try:
            # 원문 보강: 제목/RSS요약만으로는 내용이 빈약하므로, 실제 원문을 최대한 가져와서
            # 캡션 작성 재료로 사용한다. 실패해도 기존 요약으로 진행되므로 전체 파이프라인은 안 멈춤.
            if country == "KR":
                enriched = kr_sources.enrich_kr_with_naver(a.title)
            else:
                enriched = base.fetch_article_body(a.url)
            source_material = enriched or a.summary or a._image_hint

            content = generate_card_content(a.title, source_material, country, a.category_hint)
            time.sleep(LLM_CALL_INTERVAL_SEC)
            prompt = content["image_prompt"]
            bg = imagegen.generate_image(prompt)
            final_img = postprocess.build_card(bg, content["headline"], country=country)
